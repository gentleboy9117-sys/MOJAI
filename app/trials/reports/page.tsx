...
function isRealUrl(url?: string | null): boolean {
  return (
    !!url &&
    /^https?:\/\//i.test(url) &&
    !/(example|\.invalid|\/sample\/|localhost)/i.test(url)
  );
}
...
export default function TrialReportsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const period =
      dates.length > 0
        ? `${formatDate(new Date(Math.min(...dates)).toISOString())} ~ ${formatDate(
            new Date(Math.max(...dates)).toISOString(),
          )}`
        : "-";
    const topOffices = topCount(
      rows.map((r) => r.primaryOfficeName ?? "").filter(Boolean),
      5,
    );
    const bySubtype = topCount(
      rows.map((r) => r.crimeSubtype || "기타"),
      99,
    );
    const notable = [...rows]
      .sort((a, b) => {
    ...
    <div className="mx-auto max-w-content space-y-5 p-5">
      <div>
        <h1 className="flex items-center gap-2 text-heading-m text-ink-title">
          <FileText className="h-5 w-5 text-primary" /> 공판 브리핑
        </h1>
        <p className="text-body-s text-ink-muted">
          공판 보도 요약 브리핑 (공개 보도 기준)
        </p>
      </div>
      <TrialSubNav />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          desc="공판 관련 기사가 수집되면 브리핑이 생성됩니다."
        />
      ) : (
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle>기저 범죄유형 분포</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-1.5">
              ...
              {summary.notable.map((a, i) => (
                ...
                >
                  <div className="mb-1 flex flex-wrap items-center gap-1.5">
                    ...
                    <Badge tone="navy">{a.primaryOfficeName ?? "관할 추정"}</Badge>
                    <Badge tone="blue">{a.crimeSubtype || "기타"}</Badge>
                    <span className="text-detail text-ink-muted">
                      파급도 {a.issueScore}/100 · {formatDate(a.publishedAt)}
                    </span>
                  </div>
                  ...
                  {isRealUrl(a.originalUrl) ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-1 inline-flex items-center gap-1 text-detail font-medium text-blue-60 hover:underline"
                    >
                      원문 보기 <ExternalLink className="h-3 w-3" />
                    </a>
                  ) : (
                    <span className="mt-1 inline-block text-detail text-ink-disabled">
                      원문 링크 없음(샘플)
                    </span>
                  )}
                </div>
